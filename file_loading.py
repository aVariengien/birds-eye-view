...
def load_txt(file_name: str, max_chunk: Optional[int] = 100, chunk_size: int = 800) -> List[Chunk]:
    """
...
def import_pdf(file_name: str, max_chunk: Optional[int] = 100, chunk_size: int = 800) -> List[Chunk]:
    ...
    chunks = [
        Chunk(og_text=text, attribs={"page": page_nb[i], "index": i, "title": file_name})
...
def load_url(url: str, max_chunk: Optional[int] = 100, chunk_size: int = 800) -> List[Chunk]:
    """
    ...
    chunks = [Chunk(og_text=text, attribs={"title": title, "url": url, "index": i}) for i,text in enumerate(texts)]
    return chunks

def load_file(file_name: str, max_chunk: Optional[int] = 100, chunk_size: int = 800) -> List[Chunk]:
    """
...
def load_files(file_paths: List[str], max_chunk: Optional[int] = 100, chunk_size: int = 800) -> List[Chunk]:
    """
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        future_to_file = {executor.submit(load_file, file_path, max_chunk, chunk_size): file_path for file_path in file_paths}
